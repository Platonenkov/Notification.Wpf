...
namespace Notifications.Wpf.ViewModels
{
    ...
    public class NotificationProgressViewModel : ViewModel, ICustomizedNotification
    {
                    ...
                    notification.TitleTextSettings, notification.MessageTextSettings)
        {
            ...
            OnProgress(new NotificationProgressReport(null, Message, Title, showCancelButton));
        }
            ...
            TextContentSettings messageTextSettings = null)
        {
            ...
            NotifierProgress = new NotifierProgress<NotificationProgressReport>(OnProgress);
        ...
        private NotifierProgress<NotificationProgressReport> _NotifierProgress;

        /// <summary>Прогресс</summary>
        public NotifierProgress<NotificationProgressReport> NotifierProgress
        {
        ...
        void OnProgress(NotificationProgressReport report)
        {
            if (_Timer.ElapsedMilliseconds < 100 && report.Value is not null && report.Value != 100 && report.Value != 0)
                return;
            _Timer.Restart();
            string wait_message = NotifierProgress.WaitingTimer.BaseWaitingMessage;
            ...
            else
            {
                ...
                process = (double)report.Value;
                if (wait_message is null or "")
                    WaitingTime = null;
                else if (process > 10)
                {
                    WaitingTime = NotifierProgress.WaitingTimer.GetStringTime((double)report.Value, 100);
            ...
            if (report.Title != null) Title = report.Title;
            if (report.ShowCancel != null)
                ShowCancelButton = (bool)report.ShowCancel;
        }
